...
local hub_name = get_hub_name()

local function save_config()
    if not isfolder(hub_name) then
        makefolder(hub_name)
    end

    if not isfolder(`{hub_name}/configs`) then
        makefolder(`{hub_name}/configs`)
    end

    if not isfile(`{hub_name}/configs/{game.PlaceId}.json`) then
        writefile(`{hub_name}/configs/{game.PlaceId}.json`, HttpService:JSONEncode({}))
    end
    ...
    writefile(`{hub_name}/configs/{game.PlaceId}.json`, HttpService:JSONEncode(Config))
end

local function fetcher_config()
    if not isfolder(hub_name) or not isfile(`{hub_name}/configs/{game.PlaceId}.json`) then
        save_config()
    end

    Fetcher = HttpService:JSONDecode(readfile(`{hub_name}/configs/{game.PlaceId}.json`))
...
local Window = Fluent:CreateWindow({
    Title = hub_name,
    SubTitle = (function()
        local custom_subtitle = getgenv().sets.sub_title

        if not custom_subtitle then
            return ''
        end
    ...
    TabWidth = (function()
        local custom_width = getgenv().sets.tab_width

        if not custom_width then
            return 110
        end
    ...
    Size = (function()
        local custom_size = getgenv().sets.window_size

        if not custom_size then
            return UDim2.fromOffset(480, 360)
        end

        local x = custom_size.x
        local y = custom_size.y
    ...
    Acrylic = (function()
        local custom_acrylic = getgenv().sets.acrylic

        if not custom_acrylic then
            return false
        end

        return custom_acrylic
    end)(),
    Theme = (function()
        local custom_theme = getgenv().sets.theme

        if not custom_theme or custom_theme == '' then
            return 'Darker'
        end

        return custom_theme:gsub('^%l', string.upper)
    end)(),
    MinimizeKey = (function()
        local custom_key = getgenv().sets.keybind

        if not custom_key or custom_key == '' then
            return Enum.KeyCode.End
        end

        local success, key = pcall(function()
            return Enum.KeyCode[custom_key:gsub('%s+', ''):lower():gsub('(%a)(%w*)', function(a, b)
